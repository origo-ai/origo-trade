...
import { BACKEND_DIR, DB_PATH } from "../config/env.js";
...
function runMigrations(db) {
  ...
  const migrationDir = path.join(BACKEND_DIR, "server", "migrations");
  const files = fs
...
export function getDb() {
  ...
  if (!databaseConstructor) {
    ...
    databaseConstructor = loadedModule.default ?? loadedModule;
  }
  ensureDbDir();
