...
import { getInventorySnapshot } from "@/services/customer-api/client";
...
export default function Inventory() {
  ...
  const fetchStock = useCallback(async () => {
    ...
    try {
      const bridgeData = await getInventorySnapshot({ email, username });
      setRows(bridgeData.rows);
      setLoading(false);
      return;
    } catch (bridgeError) {
      setError(
        bridgeError instanceof Error ? bridgeError.message : "Unable to load inventory data.",
      );
      setRows([]);
      setLoading(false);
    }
