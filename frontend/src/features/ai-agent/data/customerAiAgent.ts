...
import {
  ...
  getOrdersShipmentsSnapshot,
} from "@/services/customer-api/client";
...
export async function loadAgentDataSnapshot(email: string, username: string): Promise<AgentDataSnapshot> {
  ...
  const context = await getCustomerPortalContext(identity);
  const customer: CustomerRow = {
    id: context.customerId || context.matchedProfileEmail || email || username || "customer-account",
    company_name: context.companyName || "Customer Account",
    email: context.matchedProfileEmail || email,
    contact_name: context.companyName || username || "Customer Contact",
    phone: "-",
    country: "-",
  ...
  const [ordersResult, inventoryResult, invoicesResult, marketResult] = await Promise.allSettled([
    getOrdersShipmentsSnapshot(identity),
    getInventorySnapshot(identity),
    getInvoicesSnapshot(identity),
    getMarketIntelligenceSnapshot(identity),
  ]);

  const ordersSnapshot = ordersResult.status === "fulfilled" ? ordersResult.value : null;
  if (ordersResult.status === "rejected") {
    warnings.push(`orders_shipments bridge failed: ${extractErrorMessage(ordersResult.reason)}`);
  }

  const inventorySnapshot = inventoryResult.status === "fulfilled" ? inventoryResult.value : null;
  if (inventoryResult.status === "rejected") {
    warnings.push(`inventory bridge failed: ${extractErrorMessage(inventoryResult.reason)}`);
  }

  const invoicesSnapshot = invoicesResult.status === "fulfilled" ? invoicesResult.value : null;
  if (invoicesResult.status === "rejected") {
    warnings.push(`invoices bridge failed: ${extractErrorMessage(invoicesResult.reason)}`);
  }

  const marketSnapshot = marketResult.status === "fulfilled" ? marketResult.value : null;
  if (marketResult.status === "rejected") {
    warnings.push(`market_intelligence bridge failed: ${extractErrorMessage(marketResult.reason)}`);
  }

  const uploads: UploadRow[] = [];
  const marketCompanyRows = marketSnapshot?.companyRows ?? [];
  let market: PurchaseTrendRow[] = (marketSnapshot?.marketRows ?? []).map((row) => ({
    date: toStringOrNull(row.date),
    weight_kg:
      typeof row.weight_kg === "number" || typeof row.weight_kg === "string"
        ? row.weight_kg
        : typeof row.weight === "number" || typeof row.weight === "string"
          ? row.weight
          : null,
    destination_country:
      toStringOrNull(row.destination_country) ??
      toStringOrNull(row.country) ??
      toStringOrNull(row.customer_location),
    product: toStringOrNull(row.product) ?? toStringOrNull(row.product_name),
    importer:
      toStringOrNull(row.importer) ??
      toStringOrNull(row.customer_name) ??
      toStringOrNull(row.customer),
  }));
  if (market.length === 0 && marketCompanyRows.length > 0) {
    market = marketCompanyRows.map((row) => ({
      date:
        toStringOrNull(row.latest_purchase_time) ??
        toStringOrNull(row.latest_purchase_date) ??
        toStringOrNull(row.created_at),
      weight_kg:
        typeof row.trades === "number" || typeof row.trades === "string"
          ? row.trades
          : typeof row.supplier_number === "number" || typeof row.supplier_number === "string"
            ? row.supplier_number
            : null,
      destination_country:
        toStringOrNull(row.customer_location) ??
        ...
        toStringOrNull(row.country),
      product:
        toStringOrNull(row.product) ??
        toStringOrNull(row.product_name) ??
        toStringOrNull(row.product_description),
      importer:
        toStringOrNull(row.customer_name) ??
        toStringOrNull(row.customer) ??
        toStringOrNull(row.company_name),
    }));
    warnings.push("market intelligence uses company-level fallback data because purchase trend rows are unavailable.");
  }
  const stock: StockRow[] = (inventorySnapshot?.rows ?? []).map((row) => ({
    factory: row.factory,
    qty: row.qty,
    type: row.type,
    tag: row.tag,
  ...
  const invoices: InvoiceRow[] = (invoicesSnapshot?.rows ?? []).map((row) => ({
    invoice: row.invoice,
    usd: row.usd,
  ...
  const supplemental = Object.fromEntries(
    SUPPLEMENTAL_TABLES.map((table) => [table, [] as GenericRow[]]),
  ) as Record<string, GenericRow[]>;

  supplemental.contract_lines = (ordersSnapshot?.contractRows ?? []).map((row) => ({
    line_id: row.id,
    customer_id: customer.id,
    contract_id: row.contractId,
    job: row.jobValue ?? row.job,
    status: row.status,
    ton: row.ton,
    acc: row.acc,
    date_to: row.dateTo,
    contracts: { customer: row.customer },
  }));

  supplemental.deliveries = (ordersSnapshot?.deliveryRows ?? []).map((row) => ({
  ...
  supplemental.finance_invoices = invoices.map((row) => ({
    customer_id: customer.id,
    invoice: row.invoice,
    usd: row.usd,
  ...
  if (marketCompanyRows.length) {
    const companyKey = marketSnapshot?.companySource === "supabase_companies" ? "supabase_companies" : "companies";
    supplemental[companyKey] = marketCompanyRows;
  }
