...
import { getInvoicesSnapshot } from "@/services/customer-api/client";
...
export default function InvoicesPayments() {
  ...
  useEffect(() => {
    ...
    const fetchInvoices = async () => {
      ...
      try {
        const bridgeData = await getInvoicesSnapshot({ email, username });
        if (cancelled) return;
        ...
        if (cancelled) return;
        setError(
          bridgeError instanceof Error ? bridgeError.message : "Unable to load invoice data.",
        );
        setRows([]);
        setLoading(false);
      }
