...
import { getCustomerPortalContext } from "@/services/customer-api/client";
...
export type CustomerScope = {
  ...
  source: "bridge_context" | "users_customer_id" | "customers_email" | "customers_username" | "none";
};
...
export const isLegacySchemaUnavailableError = (error: unknown) => (
  isMissingCustomerScopeColumnError(error) || isSchemaError(parseErrorMessage(error))
);

export async function resolveCustomerScope(input: { email?: string; username?: string } = {}): Promise<CustomerScope> {
  try {
    const bridged = await getCustomerPortalContext(input);
    if (bridged.customerId) {
      return {
        customerId: bridged.customerId,
        companyName: bridged.companyName ?? null,
        source: "bridge_context",
      };
    }
  } catch {
    // Fall back to the legacy direct-Supabase resolver when the local API bridge is unavailable.
  }
  ...
  if (authUserId) {
    const bridgeProfileResponse = await supabase
      .from("user_profiles")
      .select("default_entity_id, full_name, email")
      .eq("user_id", authUserId)
      .maybeSingle();

    if (!bridgeProfileResponse.error && bridgeProfileResponse.data) {
      const bridgeProfile = bridgeProfileResponse.data as Record<string, unknown>;
      const bridgeCustomerId = normalizeText(bridgeProfile.default_entity_id);
      if (bridgeCustomerId) {
        return {
          customerId: bridgeCustomerId,
          companyName: normalizeText(bridgeProfile.full_name) || null,
          source: "bridge_context",
        };
      }
      if (!normalizedEmail) {
        normalizedEmail = normalizeText(bridgeProfile.email).toLowerCase();
      }
    }
