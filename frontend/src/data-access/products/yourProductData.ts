...
import { getYourProductSnapshot } from "@/services/customer-api/client";
...
export async function loadProductRequests(options: LoadProductRequestsOptions = {}) {
  ...
  const filterScopedRows = (rows: ProductRequestRecord[], fallbackEmail = "") => {
    if (normalizedCustomerId) {
      const byCustomerId = rows.filter((row) => normalizeText(row.customer_id) === normalizedCustomerId);
      if (byCustomerId.length > 0) return byCustomerId;
    }

    const effectiveEmail = normalizedCustomerEmail || fallbackEmail;
    if (effectiveEmail) {
      const byEmail = rows.filter((row) => row.customer_email.trim().toLowerCase() === effectiveEmail);
      if (byEmail.length > 0) return byEmail;
    }

    if (normalizedCustomerId || effectiveEmail) {
      return [];
    }
  ...
  if (error) {
    if (isSchemaError(error.message)) {
      ...
      const bridgeEmail = normalizeText(authUserResponse.data.user?.email).toLowerCase();

      if (normalizedCustomerEmail || bridgeEmail) {
        try {
          const bridgeData = await getYourProductSnapshot({
            email: normalizedCustomerEmail || bridgeEmail,
          });
          const bridgeRows = filterScopedRows(
            bridgeData.rows.map((row) => normalizeRequestRow(row as Record<string, unknown>)),
            bridgeEmail,
          );
          const localRows = filterScopedRows(loadLocalRequests(), bridgeEmail);

          if (bridgeRows.length > 0 || localRows.length === 0) {
            const merged = [
              ...bridgeRows,
              ...localRows.filter((row) => !bridgeRows.some((bridgeRow) => bridgeRow.id === row.id)),
            ].sort((a, b) => b.submitted_at.localeCompare(a.submitted_at));
          ...
          // Fall through to local fallback when backend API is unavailable.
        }
      }

      return {
        rows: filterScopedRows(loadLocalRequests(), bridgeEmail),
